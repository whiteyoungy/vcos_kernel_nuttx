...
/****************************************************************************
 * Name: DMA_GET_CHAN_TIMEOUT
 *
 * Description:
 *   Get a DMA channel. This function gives the caller mutually exclusive
 *   access to the DMA channel specified by the 'ident' argument.
 *
 *   If the DMA channel is not available, then DMA_GET_CHAN_TIMEOUT will wait
 *   until the holder of the channel relinquishes the channel by calling
 *   DMA_PUT_CHAN() or timeout occurrs.
 *
 * Input Parameters:
 *   ident - Identifies the channel resource
 ...
 *
 * Returned Value:
 *   Provided that 'ident' is valid, this function returns a non-NULL,
 *   dma_chan_s instance, or NULL when timeout occurs
 *
 ****************************************************************************/

#define DMA_GET_CHAN_TIMEOUT(dev, ident, timeout) (dev)->get_chan_timeout(dev, ident, timeout)
...
struct dma_dev_s
{
  CODE FAR struct dma_chan_s *(*get_chan)(FAR struct dma_dev_s *dev,
                                          unsigned int ident);
  CODE FAR struct dma_chan_s *(*get_chan_timeout)(FAR struct dma_dev_s *dev,
                                          unsigned int ident, unsigned int timeout);
