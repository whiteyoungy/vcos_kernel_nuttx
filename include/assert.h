 ...
 ****************************************************************************/
#ifdef CONFIG_BMP
/* This is a global variable used to synchronize the output of the
 * assertion message with the BMP output. In BMP, when one CPU asserts, other
 * CPUs continue to work normally. To obtain a clean assert log, it is
 * necessary to temporarily prevent the other CPUs from asserting and printing
 * other logs while one CPU is printing assert log.
 */
extern volatile uint32_t g_assert_sync;
#endif
