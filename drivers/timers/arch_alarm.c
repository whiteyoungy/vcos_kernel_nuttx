...
static FAR __percpu_bss struct oneshot_lowerhalf_s *g_oneshot_lower;
#define g_oneshot_lower this_cpu_var(g_oneshot_lower)

#ifndef CONFIG_SCHED_TICKLESS
static __percpu_bss clock_t g_current_tick;
...
#ifdef CONFIG_ONESHOT_TICK_COMPENSATE
static int down_time2ticks(FAR const struct timespec *reltime,
                     FAR sclock_t *ticks)
{
  ...
  *ticks = (sclock_t)(relnsec / NSEC_PER_TICK);
  return 0;
#else
  int32_t relusec;
  ...
  *ticks = (sclock_t)(relusec / USEC_PER_TICK);
  return 0;
#endif
}
#endif
...
#ifdef CONFIG_ONESHOT_TICK_COMPENSATE
  ONESHOT_CURRENT(g_oneshot_lower, &curr);
  down_time2ticks(&curr, &now);
#else
...
#ifdef CONFIG_ONESHOT_TICK_COMPENSATE
  clock_ticks2time(&next, now + 1);
  clock_timespec_subtract(&next , &curr, &diff);
  ONESHOT_START(g_oneshot_lower, oneshot_callback, NULL, &diff);
#else
