...
#define note_wakeup(drv, tcb)                                                \
  ((drv)->ops->wakeup && ((drv)->ops->wakeup(drv, tcb), true))
...
#ifdef CONFIG_SCHED_INSTRUMENTATION_WAKEUP
void sched_note_wakeup(FAR struct tcb_s *tcb, pid_t wakeup_id)
{
  struct note_wakeup_s note;
  FAR struct note_driver_s **driver;
  bool formatted = false;
  ...
  for (driver = g_note_drivers; *driver; driver++)
    {
      ...
      if (!formatted)
        {
          formatted = true;
          note_common(tcb, &note.nwa_cmn, sizeof(struct note_wakeup_s),
                      NOTE_WAKEUP);
          note.wakeup_id = wakeup_id;
        }

      /* Add the note to circular buffer */

      note_add(*driver, &note, sizeof(struct note_wakeup_s));
    }
}
#endif /* CONFIG_SCHED_INSTRUMENTATION_WAKEUP */

#ifdef CONFIG_SCHED_INSTRUMENTATION_ABNORMAL
void sched_note_nowait(void)
{
  struct note_suspend_s susp_note;
  struct note_resume_s resum_note;
  FAR struct note_driver_s **driver;
  bool formatted = false;
  ...
  for (driver = g_note_drivers; *driver; driver++)
    {
      ...
      if (!formatted)
        {
          formatted = true;
          note_common(tcb, &susp_note.nsu_cmn, sizeof(struct note_suspend_s),
                      NOTE_SUSPEND);
          susp_note.nsu_state = FIRST_BLOCKED_STATE;

          note_common(tcb, &resum_note.nre_cmn, sizeof(struct note_resume_s),
                      NOTE_RESUME);
        }

          /* Add the note to circular buffer */

        note_add(*driver, &susp_note, sizeof(struct note_suspend_s));
        note_add(*driver, &resum_note, sizeof(struct note_resume_s));
    }
}

void sched_note_reactivate(FAR struct tcb_s *tcb)
{
  struct note_reactivate_s note;
  FAR struct note_driver_s **driver;
  bool formatted = false;
  ...
  for (driver = g_note_drivers; *driver; driver++)
    {
      ...
      if (!formatted)
        {
          formatted = true;
          note_common(tcb, &note.nreact_cmn,
                      sizeof(struct note_reactivate_s), NOTE_REACTIVATE);
        }

      /* Add the note to circular buffer */

      note_add(*driver, &note, sizeof(struct note_reactivate_s));
    }
}
#endif /* CONFIG_SCHED_INSTRUMENTATION_ABNORMAL */
                         ...
                         int type)
{
  ...
  /* NULL spinlock actually means g_irq_spin by default, to avoid crash
   * when dereference spinlock to get nsp_value.
   */
