...
static __percpu_bss char g_uart1rxbuffer[CONFIG_UART1_RXBUFSIZE];
static __percpu_bss char g_uart1txbuffer[CONFIG_UART1_TXBUFSIZE];
...
static __percpu_bss char g_uart2rxbuffer[CONFIG_UART2_RXBUFSIZE];
static __percpu_bss g_uart2txbuffer[CONFIG_UART2_TXBUFSIZE];
...
static __percpu_bss char g_uart3rxbuffer[CONFIG_UART3_RXBUFSIZE];
static __percpu_bss char g_uart3txbuffer[CONFIG_UART3_TXBUFSIZE];
...
static __percpu_data struct uart_dev_s g_uart0port =
{
  .recv =
    {
...
static __percpu_data struct uart_dev_s g_uart1port =
{
  .recv =
    {
...
static __percpu_data struct uart_dev_s g_uart2port =
{
  .recv =
    {
...
static __percpu_data struct uart_dev_s g_uart3port =
{
  .recv =
    {
...
#if defined(CONFIG_BOARD_VDK_ISP) || defined(CONFIG_BOARD_VDK_FSI) || defined(CONFIG_BOARD_VDK_NPUSCHED)
  config->uart->ibrd = 1;
  config->uart->fbrd = 0;
#else
...
static void pl011_send(FAR struct uart_dev_s *dev, int ch)
{
  ...
  while (!pl011_irq_tx_ready(sport) && retry_cycle++ < 10000);
  ...
  if (ret == OK)
    {
      ...
      if (up_cpu_index() != 0)
        {
          up_affinity_irq(sport->irq_num, 1);
        }
#endif
    }
  else
