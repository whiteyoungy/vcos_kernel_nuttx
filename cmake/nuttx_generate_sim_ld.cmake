...
set(PROCESS_SIM_LD_SCRIPT
    ...
    sed -i '/SORT(CONSTRUCTORS)/ a\
    _edata = ABSOLUTE(.);' "$target_ld"
    sed -i '/\.bss\s*:/,/{/ {
        /{/ a\
    _sbss = ABSOLUTE(.);
    }' "$target_ld"
    sed -i "/pad the \.data section\.  \*/ {
        n;  # Move to the next line after the match
        /. = ALIGN(. != 0 ? 64 \/ 8 : 1);/ a\\
    _ebss = ABSOLUTE(.);\\
    . += (_ebss - _sdata + ${idlestack}) * ${ncpus};\\
    _temp_variable = (_ebss - _sdata + ${idlestack}) * ${ncpus};
    }" "$target_ld"
...
add_custom_command(
  ...
  COMMAND
    ...
    || true
  COMMAND sh process_sim_ld_script.sh nuttx-orig.ld nuttx.ld ${CONFIG_IDLETHREAD_STACKSIZE} ${CONFIG_BMP_NCPUS}
