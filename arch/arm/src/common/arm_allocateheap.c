...
# define ROUND_UP(a, b)          (((a) + (b) - 1) & ~((b) - 1))
  size_t reserve = ROUND_UP(g_cpu_data_size, 64) * (CONFIG_BMP_NCPUS - 1);
  ssize_t heap_size_total = end - base - reserve;
  DEBUGASSERT(heap_size_total > 0);
  *heap_size = heap_size_total / CONFIG_BMP_NCPUS;
