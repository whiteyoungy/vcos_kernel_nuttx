...
#define CP15_CNTX_CVAL CP15_CNTV_CVAL
#define CP15_CNTX_CTL CP15_CNTV_CTL
#define CP15_CNTXCT CP15_CNTVCT
#elif defined(CONFIG_ARMV8R_GTIMER_PHYSICAL)
/* CNTP_CVAL, Counter-timer EL1 Physical Timer CompareValue register
 * CNTP_CTL, Counter-timer EL1 Physical Timer Control register
 */

#define CP15_CNTX_CVAL CP15_CNTP_CVAL
#define CP15_CNTX_CTL CP15_CNTP_CTL
#define CP15_CNTXCT CP15_CNTPCT
#elif defined(CONFIG_ARMV8R_GTIMER_HYPERVISOR)
/* CNTHP_CVAL, Counter-timer EL2 Physical Timer CompareValue register
 * CNTHP_CTL, Counter-timer EL2 Physical Timer Control register
 */

#define CP15_CNTX_CVAL CP15_CNTHP_CVAL
#define CP15_CNTX_CTL CP15_CNTHP_CTL
#define CP15_CNTXCT CP15_CNTPCT
#endif

#define CNTX_CTL_ENABLE_BIT         BIT(0)
#define CNTX_CTL_IMASK_BIT          BIT(1)
...
static struct arm_oneshot_lowerhalf_s * g_arm_systimer_lower_ops[CONFIG_NR_CPUS] = {0};
#endif
...
static inline void arm_arch_timer_enable(bool enable)
{
  ...
  value = CP15_GET(CNTX_CTL);

  if (enable)
    {
      value |= CNTX_CTL_ENABLE_BIT;
    }
  else
    {
      value &= ~CNTX_CTL_ENABLE_BIT;
...
#if defined(CONFIG_RT_FRAMEWORK) && (CONFIG_RT_FRAMEWORK == 1)
void up_systick_isr_handler(void)
{
  struct arm_oneshot_lowerhalf_s *priv = g_arm_systimer_lower_ops[up_cpu_index()];
...
static int arm_time_max_delay(struct oneshot_lowerhalf_s *lower,
                                  struct timespec *ts)
{
  DEBUGASSERT(ts != NULL);

  ts->tv_sec  = (time_t)UINT64_MAX;
...
static int arm_time_start(struct oneshot_lowerhalf_s *lower,
                              ...
                              const struct timespec *ts)
{
  ...
  arm_arch_timer_set_compare(arm_arch_timer_count() + timercycles);
...
static int arm_time_cancel(struct oneshot_lowerhalf_s *lower,
                               struct timespec *ts)
{
  struct arm_oneshot_lowerhalf_s *priv =
    (struct arm_oneshot_lowerhalf_s *)lower;
...
static int arm_time_current(struct oneshot_lowerhalf_s *lower,
                                struct timespec *ts)
{
  struct arm_oneshot_lowerhalf_s *priv =
    (struct arm_oneshot_lowerhalf_s *)lower;
...
int up_timer_value(FAR clock_t *ticks)
{
    (*ticks) = CP15_GET64(CNTXCT);
    return OK;
}

#if defined(CONFIG_RT_FRAMEWORK_TIMER) && (CONFIG_RT_FRAMEWORK_TIMER == 1)
static clock_t arm_hrtimer_current(struct hrtimer_queue_s *queue)
{
  (void)queue;
  return CP15_GET64(CNTXCT);
}
static void arm_hrtimer_set_compare(struct hrtimer_queue_s *queue, clock_t value)
{
  CP15_SET64(CNTX_CVAL, (uint64_t)value);
}

static void arm_htimer_start(struct hrtimer_queue_s *queue)
{
  (void)queue;
  up_enable_irq(ARM_ARCH_TIMER_IRQ);
}

static void arm_htimer_trigger(struct hrtimer_queue_s *queue)
{
  (void)queue;
  up_trigger_irq(ARM_ARCH_TIMER_IRQ, 0);
}

static const struct hrtimer_ops_s g_hrtimer_ops =
{
  .current      = arm_hrtimer_current,
  .set_expire   = arm_hrtimer_set_compare,
  .start        = arm_htimer_start,
  .trigger      = arm_htimer_trigger,
};
#endif /* CONFIG_RT_FRAMEWORK_TIMER */
#endif /* CONFIG_RT_FRAMEWORK */
static const struct oneshot_operations_s g_oneshot_ops =
{
  .max_delay      = arm_time_max_delay,
  .start          = arm_time_start,
  .cancel         = arm_time_cancel,
  .current        = arm_time_current,
...
#if defined(CONFIG_RT_FRAMEWORK) && (CONFIG_RT_FRAMEWORK == 1)
  g_arm_systimer_lower_ops[up_cpu_index()] = priv;
#endif
...
#if defined(CONFIG_RT_FRAMEWORK_TIMER) && (CONFIG_RT_FRAMEWORK_TIMER == 1)
  up_hrtimer_set_ops(&g_hrtimer_ops);
#else
