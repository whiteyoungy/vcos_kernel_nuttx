...
uint32_t *arm_prefetchabort(uint32_t *regs, uint32_t ifar, uint32_t ifsr)
{
  ...
  status_resgister_value = GET_STATUS_VALUE(ifsr);
  if (status_resgister_value == DEBUG_EXCEPTION)
    {
      bkpt_msg_len = sprintf(bkpt_msg, "Prefetch abort. PC: %08" PRIx32 " IFAR: %08" PRIx32 " IFSR: %08"
            PRIx32 "\n", regs[REG_PC], ifar, ifsr);
      set_debug_msg(regs, bkpt_msg, bkpt_msg_len);
      arm_dbgmonitor(0xC, regs, NULL);
