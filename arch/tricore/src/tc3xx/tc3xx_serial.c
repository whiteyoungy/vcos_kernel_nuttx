...
#  undef  CONFIG_UART0_SERIAL_CONSOLE
#  if defined(CONFIG_TRICORE_UART0)
#    define TTYS0_DEV       g_uart0port     /* UART0 is ttyS0 */
#    define SERIAL_CONSOLE  1
#  else
#    undef  TTYS0_DEV
...
struct up_dev_s
{
  ...
  uint8_t    rx_irq;               /* IRQ associated with this UART */
  uint8_t    tx_irq;               /* IRQ associated with this UART */
};
...
static struct up_dev_s g_uart0priv =
{
  ...
  .rx_irq       = ISR_PRIORITY_ASCLIN_RX,
  .tx_irq       = ISR_PRIORITY_ASCLIN_TX,
};
...
static void asclin_init(struct up_dev_s *priv)
{
  ...
  (void)IfxAsclin_setBitTiming(asclin,
      g_uart0priv.baud,
  ...
  /* setting Rx FIFO interrupt level at which a
   * Rx interrupt will be triggered
   */
  ...
  IfxAsclin_setClockSource(asclin, IfxAsclin_ClockSource_ascFastClock); /* select the clock source */

  IfxAsclin_disableAllFlags(asclin);                     /* disable all flags */
  IfxAsclin_clearAllFlags(asclin);                       /* clear all flags */

  IfxAsclin_enableParityErrorFlag(asclin, FALSE);
  IfxAsclin_enableFrameErrorFlag(asclin, FALSE);
  IfxAsclin_enableRxFifoOverflowFlag(asclin, FALSE);
  IfxAsclin_enableRxFifoUnderflowFlag(asclin, FALSE);
  IfxAsclin_enableTxFifoOverflowFlag(asclin, FALSE);

  /* enable transfers */

  IfxAsclin_enableRxFifoInlet(asclin, TRUE);  /* enabling Rx FIFO for recieving */
  IfxAsclin_enableTxFifoOutlet(asclin, TRUE); /* enabling Tx FIFO for transmitting */

  IfxAsclin_flushRxFifo(asclin);              /* flushing Rx FIFO */
  IfxAsclin_flushTxFifo(asclin);

  IfxAsclin_enableRxFifoFillLevelFlag(asclin, TRUE);

  IfxAsclin_enableTxFifoFillLevelFlag(asclin, TRUE);
}
...
static int up_attach(struct uart_dev_s *dev)
{
  ...
  ret = irq_attach(priv->rx_irq / 4, up_interrupt, dev);
  if (ret == OK)
    {
      ...
      ret = irq_attach(priv->tx_irq / 4, up_interrupt, dev);
      if (ret == OK)
        {
