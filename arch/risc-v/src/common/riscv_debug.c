...
{
  volatile char bkpt_msg[128] = {0};
  volatile uint32_t bkpt_msg_len = 0;
  /* Get the trigger index */

  uintreg_t cause = irq & RISCV_IRQ_MASK;
  bkpt_msg_len = sprintf(bkpt_msg, "EXCEPTION: %s. MCAUSE: %" PRIxREG ", EPC: %" PRIxREG
         ", MTVAL: %" PRIxREG "\n",
         "Breakpoint",
         cause, READ_CSR(CSR_EPC), READ_CSR(CSR_TVAL));
  set_debug_msg(context, bkpt_msg, bkpt_msg_len);
...
#if !defined(CONFIG_ARCH_CHIP_QEMU_RV) && !defined(CONFIG_ARCH_CHIP_X280)
