...
#undef g_console_rxbuf
static __percpu_bss char g_console_rxbuf[CONFIG_SIM_UART_BUFFER_SIZE];
#define g_console_rxbuf this_cpu_var(g_console_rxbuf)
#undef g_console_txbuf
static __percpu_bss char g_console_txbuf[CONFIG_SIM_UART_BUFFER_SIZE];
#define g_console_txbuf this_cpu_var(g_console_txbuf)
#endif
...
#ifdef USE_DEVCONSOLE
#undef g_console_priv
static __percpu_bss struct tty_priv_s g_console_priv ;
#define g_console_priv this_cpu_var(g_console_priv)

#undef g_console_dev
static __percpu_bss struct uart_dev_s g_console_dev ;
#define g_console_dev this_cpu_var(g_console_dev)
#endif
...
static void tty_work(void *arg)
{
  ...
  if (priv->rxint)
    {
#if defined CONFIG_NSH_SC && defined CONFIG_BMP
      extern uint32_t gactive_nsh_core;
      if (gactive_nsh_core == up_cpu_index())
#endif
#ifdef CONFIG_SIM_UART_DMA
      uart_dmarxfree(dev);
#else
...
static void tty_dmasend(struct uart_dev_s *dev)
{
  ...
  int fd = ((priv->fd == STDIN_FILENO) ? STDOUT_FILENO : priv->fd);
  int ret;
...
static void tty_send(struct uart_dev_s *dev, int ch)
{
  ...
  uart_nputs((priv->fd == STDIN_FILENO) ? STDOUT_FILENO : priv->fd, &c, 1);
}
...
static bool tty_txready(struct uart_dev_s *dev)
{
  ...
  return host_uart_checkout((priv->fd == STDIN_FILENO) ? STDOUT_FILENO : priv->fd);
}
...
/****************************************************************************
 * Name: init_console_dev
 ****************************************************************************/

void init_console_dev(void)
{
  g_console_priv.path = "console";
  g_console_priv.fd = STDIN_FILENO;
  #ifdef CONFIG_SIM_UART0_NAME
  g_console_priv.path = CONFIG_SIM_UART0_NAME;
  g_console_priv.fd = host_uart_open(g_console_priv.path);
  if (g_console_priv.fd < 0)
    {
      g_console_priv.path = "console";
      g_console_priv.fd = STDIN_FILENO;
    }
  ...
  g_console_dev.ops = &g_tty_ops;
  g_console_dev.priv = &g_console_priv;
  g_console_dev.xmit.size = CONFIG_SIM_UART_BUFFER_SIZE;
  g_console_dev.xmit.buffer = g_console_txbuf;
  g_console_dev.recv.size = CONFIG_SIM_UART_BUFFER_SIZE;
  g_console_dev.recv.buffer = g_console_rxbuf;
}
...
{
#ifdef USE_DEVCONSOLE
  int fd = (g_console_priv.fd == STDIN_FILENO) ? STDOUT_FILENO : g_console_priv.fd;
  uart_nputs(fd, str, len);
#endif
}
